...
import { ref } from 'vue';
    ...
    let page = ref(1)
    let limit = ref(1)
    let apiUrl = 'http://localhost:8222/products/'
    ...
    const  loadMoreProducts= async($state) => {
      try {
        ...
        if (newProducts.length) {
          ...
          filteredProducts = products; // Update filteredProducts with all products after loading more
          page++;
          $state.loaded();
        } else {
          $state.complete();
    ...
    const searchProducts = () => {
      if (searchQuery.trim() === '') {
        filteredProducts = products; // Reset to show all products if search query is empty
      } else {
